...
const testURLTrainerPokemons = `${baseURL}/pokemon`;
...
test("verb interp with too many operands throws err", ({ expect }) => {
  expect(() =>
    interpStringVerbs(testURLTrainerPokemons, "ash", "pikachu"),
  ).toThrow(/reads arg/v);
});

test("verb interp returns expected string", ({ expect }) => {
  const interpolated = interpStringVerbs(
    testURLTrainerPokemon,
    "ash",
    "pikachu",
  );
